...
export const addFaq = async(element)=>{
    try{
        ...
        let res = await client.mutate({
            variables: element,
            mutation : gql`
                    mutation ($file: Upload, $title: String!, $video: String, $roles: [String]!) {
                        addFaq(file: $file, title: $title, video: $video, roles: $roles) {
                            _id
                            url
                            title
                            video
...
export const setFaq = async(element)=>{
    try{
        ...
        await client.mutate({
            variables: element,
            mutation : gql`
                    mutation ($_id: ID!, $file: Upload, $title: String, $video: String, $roles: [String]) {
                        setFaq(_id: $_id, file: $file, title: $title, video: $video, roles: $roles)
                    }`})
    } catch(err){
        console.error(err)
    }
