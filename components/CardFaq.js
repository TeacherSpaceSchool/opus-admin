...
const CardFaq = React.memo((props) => {
    ...
    let _roles = ['клиент', 'исполнитель', 'гость', 'сотрудник']
                ...
                <Card className={isMobileApp?classes.cardM:classes.cardD}>
                    <CardContent>
                        ...
                        <div className={classes.row}>
                            <Button color={file?'primary':'secondary'} onClick={async()=>{faqRef.current.click()}}>
                            ...
                            element!==undefined?
                                ...
                                <Button onClick={async()=> {
                                    if (name.length > 0) {
                                        const action = async() => {
                                            setList([
                                                await addFaq({roles: roles, video: video, file: file, title: name}),
                                                ...list
                                            ])
                                        }
                                        setFile(undefined)
